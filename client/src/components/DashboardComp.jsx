...
export default function DashboardComp() {
    ...
    useEffect(()=>{
        ...
        const fetchPosts = async () => {
           try {
            const res = await fetch(`/api/post/getposts?limit=5&userId=${currentUser._id}`)
            const data = await res.json();
            if(res.ok){
                setPosts(data.posts);
                ...
                <Table hoverable>
                    ...
                    {posts && posts.map((post)=> (
                        <Table.Body key={post._id} className='divide-y'>
                            <Table.Row className='bg-white dark:border-gray-700 dark:bg-gray-800'>
                                <Table.Cell>
                                    <img src={post.image} alt='user' className='w-14 h-10  bg-gray-500'>
                                    </img>
                                </Table.Cell>
                                <Table.Cell className='w-86'>
                                    {post.title}
                                </Table.Cell>
